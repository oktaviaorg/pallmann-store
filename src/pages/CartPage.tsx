          ...
          <main className="flex-grow pt-24 pb-16">
            <div className="max-w-4xl mx-auto px-4 text-center py-16">
              <div className="bg-[#EBF4FF] w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                <ShoppingCart className="w-12 h-12 text-[#1A1A1A]" />
              </div>
              <h1 className="text-3xl font-bold text-[#1A1A1A] mb-4">Votre panier est vide</h1>
              <p className="text-[#6B6B6B] mb-8">Découvrez nos produits Pallmann professionnels</p>
              <Link
                to="/"
                className="inline-flex items-center gap-2 bg-[#FF9900] hover:bg-[#F0C300] text-white px-8 py-4 rounded-lg font-bold transition-all shadow-sm hover:shadow-md"
              >
      ...
      <div className="min-h-screen flex flex-col bg-[#F7FAFC]">
        ...
        <main className="flex-grow pt-24 pb-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold text-[#1A1A1A] mb-4 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {items.map(item => (
                  <div key={item.id} className="bg-white rounded-xl shadow-card p-6 flex gap-4 border border-gray-100">
                    ...
                    <div className="flex-grow">
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-[#EBF4FF] hover:bg-[#C3DAFE] text-[#1A1A1A] transition-colors"
                        >
                          <Minus className="w-4 h-4" />
                        </button>
                        <span className="font-bold w-8 text-center text-[#1A1A1A]">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="p-1.5 rounded-lg bg-[#EBF4FF] hover:bg-[#C3DAFE] text-[#1A1A1A] transition-colors"
                        >
                    ...
                    <div className="text-right">
                      <p className="font-bold text-lg text-[#1A1A1A]">{(item.price_ht * item.quantity).toFixed(2)}€ HT</p>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-6">
                    ...
                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      <div className="flex-grow">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-[#1A1A1A]">Livraison France</span>
                        </div>
                    ...
                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      <div className="flex-grow">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-[#1A1A1A]">Retrait sur place</span>
                        </div>
                        <p className="text-sm text-[#6B6B6B] mt-1">
                          {PICKUP_ADDRESS.full}
                        </p>
                ...
                {/* Résumé */}
                <div className="bg-white rounded-xl shadow-card p-6 border border-gray-100">
                  <h2 className="font-bold text-lg text-[#1A1A1A] mb-4">Récapitulatif</h2>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {deliveryMode === 'delivery' && shippingHT > 0 && (
                      <p className="text-xs text-[#6B6B6B]">Franco de port à partir de 630€ HT</p>
                    )}
                ...
                {/* Formulaire client */}
                <div className="bg-white rounded-xl shadow-card p-6 border border-gray-100">
                  <h2 className="font-bold text-lg text-[#1A1A1A] mb-4">Vos informations</h2>
