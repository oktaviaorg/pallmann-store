          ...
          <main className="flex-grow pt-24 pb-16">
            <div className="max-w-4xl mx-auto px-4 text-center py-16">
              <div className="bg-[#EBF4FF] w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                <ShoppingCart className="w-12 h-12 text-[#C41E3A]" />
              </div>
              <h1 className="text-3xl font-bold text-[#C41E3A] mb-4">Votre panier est vide</h1>
              <p className="text-[#64748B] mb-8">Découvrez nos produits Pallmann professionnels</p>
              <Link
                to="/"
                className="inline-flex items-center gap-2 bg-[#C41E3A] hover:bg-[#9B1830] text-white px-8 py-4 rounded-lg font-bold transition-all shadow-sm hover:shadow-md"
              >
      ...
      <div className="min-h-screen flex flex-col bg-[#F7FAFC]">
        ...
        <main className="flex-grow pt-24 pb-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold text-[#C41E3A] mb-4 flex items-center gap-3">
              <ShoppingCart className="w-8 h-8 text-[#C41E3A]" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-4">
                {items.map(item => (
                  <div key={item.id} className="bg-white rounded-xl shadow-card p-6 flex gap-4 border border-gray-100">
                    ...
                    <div className="flex-grow">
                      <h3 className="font-bold text-[#C41E3A]">{item.name}</h3>
                      <p className="text-[#C41E3A] font-bold">{item.price_ht.toFixed(2)}€ HT/{item.unit || 'L'}</p>
                      
                      <div className="flex items-center gap-3 mt-3">
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity - 1)}
                          className="p-1.5 rounded-lg bg-[#EBF4FF] hover:bg-[#C3DAFE] text-[#C41E3A] transition-colors"
                        >
                          <Minus className="w-4 h-4" />
                        </button>
                        <span className="font-bold w-8 text-center text-[#C41E3A]">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="p-1.5 rounded-lg bg-[#EBF4FF] hover:bg-[#C3DAFE] text-[#C41E3A] transition-colors"
                        >
                    ...
                    <div className="text-right">
                      <p className="font-bold text-lg text-[#C41E3A]">{(item.price_ht * item.quantity).toFixed(2)}€ HT</p>
                    </div>
                  </div>
                ))}
                ...
                <Link
                  to="/"
                  className="inline-flex items-center gap-2 text-[#9B1830] hover:text-[#C41E3A] font-semibold transition-colors"
                >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-card p-6 border border-gray-100">
                  <h2 className="font-bold text-lg text-[#C41E3A] mb-4 flex items-center gap-2">
                    ...
                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      <div className="flex-grow">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-[#C41E3A]">Livraison France</span>
                        </div>
                    ...
                    <label className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      ...
                      <div className="flex-grow">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-[#C41E3A]">Retrait sur place</span>
                        </div>
                        <p className="text-sm text-[#64748B] mt-1">
                          {PICKUP_ADDRESS.full}
                        </p>
                ...
                {/* Résumé */}
                <div className="bg-white rounded-xl shadow-card p-6 border border-gray-100">
                  <h2 className="font-bold text-lg text-[#C41E3A] mb-4">Récapitulatif</h2>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="border-t border-gray-100 pt-2 mt-2">
                      <div className="flex justify-between font-bold text-[#C41E3A]">
                        <span>Total HT</span>
                      ...
                      <div className="flex justify-between font-bold text-xl text-[#C41E3A] mt-2">
                        <span>Total TTC</span>
                ...
                {/* Formulaire client */}
                <div className="bg-white rounded-xl shadow-card p-6 border border-gray-100">
                  <h2 className="font-bold text-lg text-[#C41E3A] mb-4">Vos informations</h2>
                  ...
                  <div className="space-y-4">
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#C41E3A] focus:border-transparent text-[#2D3748]"
                      required
                    />
                    <input
                      type="text"
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full mt-6 bg-[#C41E3A] hover:bg-[#9B1830] text-white py-4 rounded-lg font-bold text-lg flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-sm hover:shadow-md"
                  >
                    {loading ? (
                      'Chargement...'
                    ) : (
